...
namespace TimeTracker.Classic.Presentation
{
    internal sealed class SettingsForm : Form
    {
        ...
        internal SettingsForm(AppSettings settings, IApplicationCatalog applicationCatalog)
        {
            ...
            _workSummaryEnabled = new CheckBox { Left = 20, Top = 120, Width = 350, Text = "Показывать шаг «Заполни итоги работы»", Checked = settings.WorkSummaryEnabled };
            Label urlLabel = new Label { Left = 40, Top = 152, Width = 80, Text = "Ссылка:" };
            _workSummaryUrl = new TextBox { Left = 120, Top = 148, Width = 290, Text = settings.WorkSummaryUrl ?? String.Empty, Enabled = settings.WorkSummaryEnabled };
            _workSummaryEnabled.CheckedChanged += delegate { _workSummaryUrl.Enabled = _workSummaryEnabled.Checked; };
            _automaticMeetingEnabled = new CheckBox { Left = 20, Top = 190, Width = 500, Text = LocalizedText.AutomaticMeetingEnabled, Checked = settings.AutomaticMeetingEnabled };
            _automaticMeetingApplications = new ListBox { Left = 40, Top = 220, Width = 510, Height = 120 };
            foreach (string path in settings.AutomaticMeetingApplications) _automaticMeetingApplications.Items.Add(path);
            Button addRunning = new Button { Left = 40, Top = 350, Width = 160, Text = LocalizedText.AddRunningApplication };
            Button addExecutable = new Button { Left = 210, Top = 350, Width = 150, Text = LocalizedText.ChooseExecutable };
            Button remove = new Button { Left = 370, Top = 350, Width = 100, Text = LocalizedText.Remove };
            ...
            Button save = new Button { Left = 380, Top = 415, Width = 85, Text = "Сохранить", DialogResult = DialogResult.OK };
            Button cancel = new Button { Left = 475, Top = 415, Width = 75, Text = "Отмена", DialogResult = DialogResult.Cancel };
            Controls.AddRange(new Control[] { _hideFromCapture, _showOnAllVirtualDesktops, _startWithWindows, _workSummaryEnabled, urlLabel, _workSummaryUrl, _automaticMeetingEnabled, _automaticMeetingApplications, addRunning, addExecutable, remove, save, cancel });
            AcceptButton = save;
            CancelButton = cancel;
        }
